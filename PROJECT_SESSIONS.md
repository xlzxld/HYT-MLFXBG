...
| 2026-09-16 ~ 09-17 | `fix-modeb-image-and-entity-switch` | **实体显示与截图链路改造**：① 实体显示按属性分类（冷流道/热流道/冷却水路/注射位置），默认都不显示，注射位置「勾了显示、不勾绝不动它」且跑完兜底保持；② 截图取景归一 `capture_settings`（fit/zoom/restore_view）+ 视角与绘图状态跑完还原，界面不留痕；③ 封面图与 Slide 2 网格图改按**类型**分离（CAD 类型 S/F/STL/BD vs 网格 T/TE/N），不再靠枚举 CAD 实体句柄；④ 实体临时挪进隐藏层截图、末尾按图层快照写回；⑤ 新增模型定向宏 `OrientModel.vbs`、冷却水路测试数据宏 `MakeCoolingCircuit*.vbs`、Moldflow API 探针 `probe_api.py`；⑥ **修复实机 800A01A8（缺少对象）中断**：定长数组元素未赋值时是 Empty，对 Empty 求 `Is Nothing` 直接中断整脚本（研究无冷流道属性时必现）→ 加 `IsEmpty` 守卫 + 回归测试。回归测试 62 → **109/109 全绿** | `AutoReport.vbs`<br>`config_gui.py`<br>`report_config.json`<br>`core/pptx_builder.py`<br>`core/image_processor.py`<br>`tests/run_checks.py`<br>`OrientModel.vbs`<br>`MakeCoolingCircuit.vbs`<br>`probe_api.py`<br>`使用说明.md`<br>`AI_GUIDE.md` |
| 2026-09-19 | `master` | **全面体检与可移植性收尾**（`fix-modeb-image-and-entity-switch` 已 fast-forward 合回 master 并推送，含此前本地领先 13 个提交）：① `report_config.json` 的 `output_dir` 个人日期绝对路径清为空串（走"模型所在目录"设计默认，VBS 不传 `--output`，端到端安全）；② `pptx_builder.py` 材料卡片字体回退分支加候选链 + `load_default()` 兜底（原先裸 `msyh.ttc` 无守卫）；③ 删除 `AutoReport.vbs` 声明未用的 `ScriptDir`（GBK 原样编辑）；④ **门禁清单补齐 `probe_api.py`**（1597 行探针此前游离在 fmt/lint 清单外），Makefile / `.agents/project.py` / AGENTS.md §2 三处同批同步；⑤ 文档纠偏：README §6 与 AI_GUIDE 坑册中"BaseDir 硬编码绝对路径"的过期描述改为 `ScriptFullName` 自动推导现状，使用说明 output_dir/template 示例改相对路径口径。门禁 109/109 全绿 | `report_config.json`<br>`core/pptx_builder.py`<br>`AutoReport.vbs`<br>`Makefile`<br>`.agents/project.py`<br>`AGENTS.md`<br>`README.md`<br>`AI_GUIDE.md`<br>`使用说明.md` |
...
4. ~~本地 `master` 领先 origin 多个提交未推送~~ **已解决（2026-09-19）**：`fix-modeb-image-and-entity-switch` 已合回 master，连同此前本地领先提交共 13 个已推送 origin/master；
