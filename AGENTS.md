...
| 构建 (Build) | 无 | — |
| 测试 (Test) | `python tests/run_checks.py` | ✅ 回归 109/109 通过；端到端冒烟（人工时跑）：`python core/pptx_builder.py --config report_config.json --data-dir temp --mode B --no-open --output temp\smoke.pptx` 输出 [SUCCESS]；已知豁免清单内且非本次引入的失败不阻断，但须列出 |
| 静态检查 (Lint) | `python -m py_compile config_gui.py core/gif_enhancer.py core/image_processor.py core/pptx_builder.py probe_api.py check_env.py tests/run_checks.py` | ✅ 0 错误 0 警告（基线豁免同上） |
| 格式化 (Format) | `python -m ruff format --check config_gui.py core/gif_enhancer.py core/image_processor.py core/pptx_builder.py probe_api.py check_env.py tests/run_checks.py` | ✅ 对本次改动文件执行，0 差异（ruff 0.16.6） |
| 聚合门禁 (闭环入口) | 无（v3 起由技能目录 `tsc.py verify --project <本项目>` 承担，不再依赖 make/Makefile） | ✅ 四段逐段退出码 0 |
