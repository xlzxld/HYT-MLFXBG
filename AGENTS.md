...
| 构建 (Build) | 无 | — |
| 测试 (Test) | `python tests/run_checks.py` | ✅ 回归 62/62 通过；端到端冒烟（人工时跑）：`python core/pptx_builder.py --config report_config.json --data-dir temp --mode B --no-open --output temp\smoke.pptx` 输出 [SUCCESS]；已知豁免清单内且非本次引入的失败不阻断，但须列出 |
